...
export const Folders: React.FC = () => {
  const { user } = useAuth();
  const { publicFolders, linkCounts, loading } = useData();
  ...
  return (
    <div className="p-6 md:p-10 max-w-6xl mx-auto pb-24">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            ...
            {publicFolders.map(f => (
              <div key={f.id} className="h-32 md:h-40">
                <FolderCard folder={f} linkCount={linkCounts[f.id] || 0} />
              </div>
            ))}
          </div>
      ...
      <AddFolderModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} userId={user?.uid || ''} />
    </div>
  );
};